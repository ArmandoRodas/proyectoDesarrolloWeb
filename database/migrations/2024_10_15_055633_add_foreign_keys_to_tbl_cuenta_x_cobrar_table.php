...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('tbl_cuenta_x_cobrar', function (Blueprint $table) {
            $table->foreign(['id_cliente'], 'tbl_cuenta_x_cobrar_ibfk_1')->references(['id_persona'])->on('tbl_persona')->onUpdate('no action')->onDelete('no action');
            $table->foreign(['id_estado'], 'tbl_cuenta_x_cobrar_ibfk_2')->references(['id'])->on('tbl_estado')->onUpdate('no action')->onDelete('no action');
